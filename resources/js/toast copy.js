...
export function initializeToastListeners() {
    ...
    Livewire.on('status-updated', (event) => {
        showToast(`Status updated to "${event.status}" for ${event.name}`, 'success');
    });
    ...
    Livewire.on('toast-message', (event) => {
        console.log('Toast message received:', event);
        const data = event[0]; // اولین المنت آرایه رو می‌گیریم
